...
<template>
  <view class="loginBox">
    ...
    >
      ...
      <view class="content">
        <view>
          在您注册成为白鹭到家会员的过程中您需要通过点击同意的形式在线签署
          <text>《白鹭到家服务条款》</text>
          、
          <text>《白鹭到家隐私政策》</text>
...
import { getAddress, getCityList, getIdByCityCode } from '../api/address'
// ------定义变量------
...
const getCity = (position) => {
  ...
  getAddress(params).then((res) => {
    if (res.data.code === 200) {
      ...
      getIdByCityCode({ CityCode: currentCity.value.cityCode }).then((res) => {
        if (res.data.code === 200) {
