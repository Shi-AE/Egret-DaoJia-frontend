...
<template>
    <view class="pageCon appBox verifyBox">
        ...
        ></NavBar>
        <!-- end -->
        <view v-if="netStatus" class="main">
            <view class="headBox">
                <view class="time"
                >支付剩余时间
                    <text>{{ times }}</text>
                ...
                <view class="tag">
                    ...
                    <view
                    >
                        <text class="cheap">{{ (price - cheapPrice).toFixed(2) }}元
                        </text
                        >
                    </view
                    >
            ...
            <view class="pageFoot">
                <view>
                    还需支付：
                    <text>{{ (price - cheapPrice).toFixed(2) }}元</text>
                </view>
                <button class="agree-btn btn" @click="getTradeImg">确认支付</button>
            </view>
        </view>
        <uni-popup
            ref="popup"
