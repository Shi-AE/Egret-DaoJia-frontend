...
<template>
    <view class="page-content">
        ...
        <!-- 选择器主体 -->
        <view :style="'height:60%'" class="page-main">
            <!-- 操作区域 包含取消和确定 -->
            ...
            <!-- 已选择的地址信息 -->
            <view :class="[selectAreaLevelLimit == 3 ? 'w33_percent' : '']" class="main-select">
                <text
                    v-if="showIndex == 0 || provinceObj.areaName"
                    :class="showIndex == 0 ? 'select' : ''"
                    @click="anewSelect(0)"
                >{{ provinceObj.areaName || "请选择" }}
                </text
                >
                <text
