<!-- 头部导航组件 -->
<template>
    <view :style="{ height: all }" class="NavBar">
        <view
            :style="{ paddingTop: capsuleTop + 'px', height: capsuleHeight + 'px' }"
            class="title"
        >{{ title
            }}
            <view v-if="isShowBack" class="back" @click="handleTo">
                <image :src="src"></image>
            </view>
            <view v-if="isHistory && historytime !== true" class="history" @click="handleToHistory">
