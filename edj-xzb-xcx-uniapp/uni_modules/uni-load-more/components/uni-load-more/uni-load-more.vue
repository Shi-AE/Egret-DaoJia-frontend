<template>
    <view class="uni-load-more" @click="onClick">
        ...
        <!-- #endif -->
        <!-- #ifdef H5 -->
        <svg v-if="!webviewHide && (iconType==='circle' || iconType==='auto' && platform === 'android') && status === 'loading' && showIcon" :style="{width:iconSize+'px',height:iconSize+'px'}" class="uni-load-more__img uni-load-more__img--android-H5"
             height="24"
             viewBox="25 25 50 50"
             width="24">
        ...
        <text v-if="showText" :style="{color: color}"
              class="uni-load-more__text">{{ status === 'more' ? contentdownText : status === 'loading' ? contentrefreshText
            : contentnomoreText }}
        </text>
    </view>
</template>
