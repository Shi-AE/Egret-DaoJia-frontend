<template>
    <view class="filtrateBox">
        <view v-for="(item, index) in itemData" :key="index" :class="isActive === index||(item==='超时任务'&&users.filterOverTime) ? 'onHover' : ''" class="item"
              @click="handleTab(index)">
            <text>{{ item }}</text>
            <icon v-if="index !== 2" :class="isActive === index && isSelect ? 'hover' : ''" class="up"></icon>
            <icon v-if="index !== 2" :class="isActive === index && !isSelect ? 'hover' : ''" class="down"></icon>
        </view>
    </view>
</template>
