        ...
        <uni-popup ref="popup" background-color="#fff" type="bottom">
            <view class="popup-content">
                ...
                <view class="date-select address">
                    <view>
                        <picker-view :indicator-style="indicatorStyle" :value="defaultValue" class="picker-view"
                                     @change="getProvincesData">
                            <!-- 省 -->
                            <picker-view-column>
                                <view v-for="(item, index) in provinceData.value" :key="index" class="item">
                                    <text>{{ item.name }}</text>
                                </view>
                            </picker-view-column>
                            <!-- end -->
                            <!-- 市 -->
                            <picker-view-column>
                                <view v-for="(item, index) in cityData.value" :key="index" class="item">
                                    <text>{{ item.name }}</text>
                                </view>
                            </picker-view-column>
                            <!-- end -->
                            <!-- 区 -->
                            <picker-view-column>
                                <view v-for="(item, index) in areaData.value" :key="index" class="item">
                                    <text>{{ item.name }}</text>
                                </view>
                            </picker-view-column>
                            <!-- end -->
